...
When enabled, STAR runs as a separate job for every sample. Each sample's
HTSeq and alignment-based QC jobs wait for its STAR job. Cohort downsampling,
mapping, and splice-junction jobs wait for all STAR jobs; FastQC and annotation
setup do not wait for alignment. Annotation setup always runs once.
Mapping-statistics parsing and splice-junction summarization run once for the
full cohort when enabled; FastQC, duplication, insert size distribution, gene
body coverage, read distribution, strandedness, exon-intron drop-off, and
Kraken run as one job per sample when enabled.
Only duplication and gene-body coverage wait for and use the BAM selected by
downsampling; HTSeq and other BAM-based modules use the full genomic BAM,
either supplied or generated by STAR. HTSeq never combines samples inside its
per-sample jobs; `Aggregate.sh` builds the cohort matrices after counting.
The final MultiQC and aggregation steps use `afterany` dependencies so that summary
reports can still be generated even if one QC module fails. Jobs whose
dependencies cannot succeed are cancelled with `--kill-on-invalid-dep=yes`
instead of remaining pending and blocking reporting.
...
| `Aggregate.sh` | Resolved `SAMPLESHEET`; available QC outputs; optional MultiQC report; current-run HTSeq counts when enabled | `summary/hpc_qc_summary.tsv`, `summary/hpc_qc_transfer_bundle.zip`, and complete HTSeq matrices | Combines QC metrics and packages the MultiQC report/data. With HTSeq enabled, combines all samples' counts or includes a missing/invalid-count notice. With HTSeq disabled, produces the QC bundle without counts. |
